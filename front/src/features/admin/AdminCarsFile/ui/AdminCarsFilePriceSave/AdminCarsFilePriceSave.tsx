...
import { createFilePrice, updateFilePrice } from '~features/admin/AdminCarsFile/api/api';
...
export interface AdminCarsFilePriceSaveProps {
  type: 'add' | 'edit';
  filePriceItem?: FilePriceItem | null;
  reFetchFilePrice?: any;
  selectedVehicle: number;
}

export const AdminCarsFilePriceSave: FC<AdminCarsFilePriceSaveProps> = ({
  type = 'add',
  filePriceItem,
  reFetchFilePrice,
  selectedVehicle,
  ...
  useEffect(() => {
    if (type === 'edit' && modal && filePriceItem) {
  ...
  const onFinish = async (values: any) => {
    ...
    try {
      ...
      if (type === 'add') {
        await createFilePrice(formData, values.id_vehicle, values.title);
      } else if (type === 'edit' && filePriceItem) {
        await updateFilePrice({ ...formData, title: values.title }, filePriceItem.id_file);
      }
