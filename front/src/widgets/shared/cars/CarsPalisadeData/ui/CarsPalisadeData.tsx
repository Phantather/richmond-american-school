...
import {
  CarsPalisadeComfort,
  CarsPalisadeDesign,
  CarsPalisadeEfficiency,
  CarsPalisadeFeatures,
  CarsPalisadePrice,
  CarsPalisadeSafety,
  CarsPalisadeSpecifications,
} from '~entities/cars';
import { usePalisadeFileInfo, useSetPalisadeFileInfo } from '~entities/shared/file-info/model';
import { usePalisadeFilePrice, useSetPalisadeFilePrice } from '~entities/shared/file-price/model';
import { CarsReq, DownLoadIcon, useWindowInnerWidth } from '~shared/ui';

import bg from '~shared/assets/palisade/banner.jpg';

export const CarsPalisadeData: FC = () => {
  ...
  const fileInfo = usePalisadeFileInfo();
  const setFileInfo = useSetPalisadeFileInfo();

  const filePrice = usePalisadeFilePrice();
  const setFilePrice = useSetPalisadeFilePrice();
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (!fileInfo) {
          await setFileInfo({ id_vehicle: 5 });
        }

        if (!filePrice) {
          await setFilePrice({ id_vehicle: 5 });
        }
  ...
  const filePriceLisView = () => {
    if (filePrice?.data?.length) {
      return <CarsPalisadePrice list={filePrice?.data} />;
    }
    ...
    <div className="pb-[100px]">
      ...
      >
        <div className="max-w-[1024px] mx-auto w-full px-5">
          <h1 className="text-white text-[60px] xs:text-[50px] font-bold">PALISADE</h1>
        </div>
      </div>
      <CarsReq />
      ...
      <div className="max-w-[1024px] mx-auto w-full px-5">
        {activeTab === 'features' && <CarsPalisadeFeatures />}
        {activeTab === 'prices' && <>{filePriceLisView()}</>}
        {activeTab === 'design' && <CarsPalisadeDesign />}
        {activeTab === 'efficiency' && <CarsPalisadeEfficiency />}
        {activeTab === 'safety' && <CarsPalisadeSafety />}
        {activeTab === 'comfort' && <CarsPalisadeComfort />}
        {activeTab === 'specifications' && <CarsPalisadeSpecifications />}
      </div>
    </div>
  );
};
