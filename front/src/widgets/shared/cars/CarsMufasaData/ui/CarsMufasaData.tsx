...
import bg from '~shared/assets/mufasa/banner.png';
import {
  CarsMufasaComfort,
  CarsMufasaDesign,
  CarsMufasaFeatures,
  CarsMufasaPrice,
  CarsMufasaSafety,
  CarsMufasaSpecifications,
} from '~entities/cars/mufasa';

export const CarsMufasaData: FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (!fileInfo) {
          await setFileInfo({ id_vehicle: 7 });
        }

        if (!filePrice) {
          await setFilePrice({ id_vehicle: 7 });
        }
  ...
  const filePriceLisView = () => {
    if (filePrice?.data?.length) {
      return <CarsMufasaPrice list={filePrice?.data} />;
    }
    ...
    <div className="pb-[100px]">
      ...
      >
        <div className="max-w-[1024px] mx-auto w-full px-5">
          <h1 className="text-white text-[60px] xs:text-[50px] font-bold">MUFASA</h1>
        </div>
      </div>
      <CarsReq />
      ...
      <div className="max-w-[1024px] mx-auto w-full px-5">
        {activeTab === 'features' && <CarsMufasaFeatures />}
        {activeTab === 'prices' && <>{filePriceLisView()}</>}
        {activeTab === 'design' && <CarsMufasaDesign />}
        {activeTab === 'safety' && <CarsMufasaSafety />}
        {activeTab === 'comfort' && <CarsMufasaComfort />}
        {activeTab === 'specifications' && <CarsMufasaSpecifications />}
      </div>
    </div>
  );
};
