...
import {
  CarsElantraComfort,
  CarsElantraDesign,
  CarsElantraEfficiency,
  CarsElantraFeatures,
  CarsElantraSafety,
  CarsElantraSpecifications,
} from '~entities/cars';
import { CarsElantraPrice } from '~entities/cars/elantra/CarsElantraPrice';
import { useElantraFileInfo, useSetElantraFileInfo } from '~entities/shared/file-info/model';
import { useElantraFilePrice, useSetElantraFilePrice } from '~entities/shared/file-price/model';
import { CarsReq, DownLoadIcon, useWindowInnerWidth } from '~shared/ui';

import bg from '~shared/assets/elantra/banner.png';

export const CarsElantraData: FC = () => {
  ...
  const fileInfo = useElantraFileInfo();
  const setFileInfo = useSetElantraFileInfo();

  const filePrice = useElantraFilePrice();
  const setFilePrice = useSetElantraFilePrice();
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (!fileInfo) {
          await setFileInfo({ id_vehicle: 1 });
        }

        if (!filePrice) {
          await setFilePrice({ id_vehicle: 1 });
        }
  ...
  const filePriceLisView = () => {
    if (filePrice?.data?.length) {
      return <CarsElantraPrice list={filePrice?.data} />;
    }
    ...
    <div className="pb-[100px]">
      ...
      >
        <div className="max-w-[1024px] mx-auto w-full px-5">
          <h1 className="text-white text-[60px] xs:text-[50px] font-bold">ELANTRA</h1>
        </div>
      </div>
      <CarsReq />
      ...
      <div className="max-w-[1024px] mx-auto w-full px-5">
        {activeTab === 'features' && <CarsElantraFeatures />}
        {activeTab === 'prices' && <>{filePriceLisView()}</>}
        {activeTab === 'design' && <CarsElantraDesign />}
        {activeTab === 'efficiency' && <CarsElantraEfficiency />}
        {activeTab === 'safety' && <CarsElantraSafety />}
        {activeTab === 'comfort' && <CarsElantraComfort />}
        {activeTab === 'specifications' && <CarsElantraSpecifications />}
      </div>
    </div>
  );
};
