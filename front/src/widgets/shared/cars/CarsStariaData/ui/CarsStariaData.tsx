...
import {
  CarsStariaComfort,
  CarsStariaDesign,
  CarsStariaEfficiency,
  CarsStariaFeatures,
  CarsStariaPrice,
  CarsStariaSafety,
  CarsStariaSpecifications,
} from '~entities/cars';
import { useSetStariaFileInfo, useStariaFileInfo } from '~entities/shared/file-info/model';
import { useSetStariaFilePrice, useStariaFilePrice } from '~entities/shared/file-price/model';
import { CarsReq, DownLoadIcon, useWindowInnerWidth } from '~shared/ui';

import bg from '~shared/assets/staria/banner.png';

export const CarsStariaData: FC = () => {
  ...
  const fileInfo = useStariaFileInfo();
  const setFileInfo = useSetStariaFileInfo();

  const filePrice = useStariaFilePrice();
  const setFilePrice = useSetStariaFilePrice();
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (!fileInfo) {
          await setFileInfo({ id_vehicle: 6 });
        }

        if (!filePrice) {
          await setFilePrice({ id_vehicle: 6 });
        }
  ...
  const filePriceLisView = () => {
    if (filePrice?.data?.length) {
      return <CarsStariaPrice list={filePrice?.data} />;
    }
    ...
    <div className="pb-[100px]">
      ...
      >
        <div className="max-w-[1024px] mx-auto w-full px-5">
          <h1 className="text-white text-[60px] xs:text-[50px] font-bold">STARIA</h1>
        </div>
      </div>
      <CarsReq />
      ...
      <div className="max-w-[1024px] mx-auto w-full px-5">
        {activeTab === 'features' && <CarsStariaFeatures />}
        {activeTab === 'prices' && <>{filePriceLisView()}</>}
        {activeTab === 'design' && <CarsStariaDesign />}
        {activeTab === 'efficiency' && <CarsStariaEfficiency />}
        {activeTab === 'safety' && <CarsStariaSafety />}
        {activeTab === 'comfort' && <CarsStariaComfort />}
        {activeTab === 'specifications' && <CarsStariaSpecifications />}
      </div>
    </div>
  );
};
