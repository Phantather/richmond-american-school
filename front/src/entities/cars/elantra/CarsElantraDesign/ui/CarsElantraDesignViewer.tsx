...
import {
  useCarsElantraDesign,
  useSetCarsElantraDesignr,
} from '~entities/cars/elantra/CarsElantraDesign/model';

import wheel1 from '~shared/assets/elantra/gls-16-alloy.jpg';
import wheel2 from '~shared/assets/elantra/gls-17-alloy.jpg';

import color1 from '~shared/assets/elantra/abyss-black-pearl.jpg';
import color2 from '~shared/assets/elantra/amazon-grey-metallic.jpg';
import color3 from '~shared/assets/elantra/atlas-white.jpg';
import color4 from '~shared/assets/elantra/cyber-gray-metallic.jpg';
import color9 from '~shared/assets/elantra/ultimate-red-metallic.jpg';
import color6 from '~shared/assets/elantra/fluid-grey-metallic.jpg';
import color7 from '~shared/assets/elantra/intense-blue-pearl.jpg';
import color8 from '~shared/assets/elantra/meta-blue-pearl.jpg';
import color5 from '~shared/assets/elantra/ecotronic-grey-pearl.jpg';

const colors = [
  { id: 1, src: color1, title: 'Черный / Abyss Black Pearl' },
  { id: 2, src: color2, title: 'Серый / Amazon Gray Metallic' },
  { id: 3, src: color3, title: 'Белый / Atlas White' },
  { id: 4, src: color4, title: 'Серый / Cyber Gray Metallic' },
  { id: 5, src: color5, title: 'Серый / Ecotronic Gray Pearl' },
  { id: 6, src: color6, title: 'Серый / Fluid Gray Metallic' },
  { id: 7, src: color7, title: 'Синий / Intense Blue Pearl' },
  { id: 8, src: color8, title: 'Синий / Meta Blue Pearl' },
  { id: 9, src: color9, title: 'Красный / Ultimate Red Metallic' },
];

const wheels = [
  { id: 1, src: wheel1, title: '16-дюймовые легкосплавные диски' },
  { id: 2, src: wheel2, title: '17-дюймовые легкосплавные диски' },
];

export const CarsElantraDesignViewer = () => {
  const elantraDesign = useCarsElantraDesign();
  const setElantraDesign = useSetCarsElantraDesignr();
  ...
  useLayoutEffect(() => {
    if (!elantraDesign) {
      ...
      setElantraDesign({ id_vehicle: 1, id_color: selectedColor, id_size: selectedWheel }).finally(
        () => setLoading(false)
      );
    }
  }, [elantraDesign, selectedColor, selectedWheel, setElantraDesign]);

  const handleColorClick = (id: any) => {
    setLoading(true);

    setSelectedColor(id);
    setElantraDesign({ id_vehicle: 1, id_color: id, id_size: selectedWheel });
  ...
  const handleWheelClick = (id: any) => {
    ...
    setSelectedWheel(id);
    setElantraDesign({ id_vehicle: 1, id_color: selectedColor, id_size: id });
  ...
  return (
    <>
      ...
      <div className="col-span-12 sm:col-span-8 xs:col-span-4 flex justify-center items-center car_rotate">
        <div className="w-[814px] h-[450px] sm:w-full sm:h-[290px] relative car_spot">
          <div className="img_cont" style={{ position: 'relative', zIndex: 9 }}>
            {images.map((i) => (
              <img
                key={i}
                src={elantraDesign?.[i]?.image_name || ''}
                alt={`elantra_view_360_deg_${i}`}
                style={{
